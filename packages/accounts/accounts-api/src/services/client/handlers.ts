...
export async function findOne(
  ...
  try {
    ...
    if (!cache.has(id)) {
      ...
      logger.info(`Fetching client with id ${id} from database`)
      ...
      logger.info(`Creating client with id ${id} in the cache`)

      cache.set(found.id, found)
    }

    logger.info(`Fetching client with id ${id} from the cache`)

    const found = cache.get(id) as Client
